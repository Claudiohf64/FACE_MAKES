...
if not cap.isOpened():
    ...
    print(f"Cámara iniciada. Solicitada: 1280x720. Obtenida: {width}x{height}")
...
print(f"Vamos a recolectar {num_images_per_gesture} imágenes para: {gestures}")
...
print("¡PREPÁRATE! La recolección comenzará en 5 segundos...")
time.sleep(5)

# --- 3. Bucle Principal de Recolección ---
...
while cap.isOpened():
    ...
    if not success:
        # Aquí es donde fallaba ("can't grab frame")
    ...
    # --- 4. Lógica de Captura y Cuenta Regresiva ---
    
    is_none_gesture = (gestures[current_gesture_index] == "none")

    # --- NUEVO: Lógica de Cuenta Regresiva ---
    if is_counting_down:
        ...
        if countdown_value > 0:
            ...
            # ¡Se acabó el tiempo! Empezar a capturar
            print("¡YA!")
            is_counting_down = False
            capturing = True
    ...
    # Lógica de guardado de fotos (solo se activa si capturing=True y pasó el delay)
    if capturing and (time.time() - last_capture_time > CAPTURE_DELAY):
        if (hand_found and not is_none_gesture) or (not hand_found and is_none_gesture):
            ...
            if roi.size > 0:
                ...
                image_path = os.path.join(gesture_folder, f"{image_count}.jpg")
    ...
    # --- NUEVO: Lógica de Controles actualizada ---
    ...
    if not capturing and not is_counting_down and current_gesture_index < len(gestures):
        ...
        if key_char == expected_key:
            if is_none_gesture:
                ...
                print(f"¡Prepárate para '{gestures[current_gesture_index].upper()}'! Empezando en {COUNTDOWN_SECONDS} segundos...")
